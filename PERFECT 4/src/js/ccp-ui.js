...
function renderCallView(doc) {
    ...
    if (appState.currentCallData) {
        if (appState.currentCallData.error) { contextContainer.innerHTML = `<div class="context-error">...</div>`; } 
            ...
            if (appState.settings.displayContactInfo) {
                contactInfoHtml = `<div class="context-card"><div class="context-item"><i class="fa-solid fa-phone-volume"></i><div class="context-text"><span class="context-label">IVR Number</span><span class="context-value">${data.ivrNumber || 'N/A'}</span></div>${data.ivrNumber ? `<i class="fa-solid fa-copy copy-icon" title="Copy IVR #" data-copy-text="${data.ivrNumber}"></i>` : ''}</div><div class="context-item"><i class="fa-solid fa-headset"></i><div class="context-text"><span class="context-label">Customer Service</span><span class="context-value">${data.csrNumber || 'N/A'}</span></div>${data.csrNumber ? `<i class="fa-solid fa-copy copy-icon" title="Copy CSR #" data-copy-text="${data.csrNumber}"></i>` : ''}</div></div>`;
            }
    ...
    // Logic for call controls is unchanged and omitted for brevity
    const singleControls = doc.getElementById('single-call-controls'); const multiControls = doc.getElementById('multi-call-controls'); const confControls = doc.getElementById('conference-controls'); const inConsultState = callCount === 2 && !appState.isConferenced; singleControls.classList.toggle('hidden', callCount !== 1); multiControls.classList.toggle('hidden', !inConsultState); confControls.classList.toggle('hidden', !appState.isConferenced); if (callCount > 0) { const holdBtn = doc.getElementById('hold-resume-btn'); const muteBtn = doc.getElementById('mute-unmute-btn'); if (appState.calls.some(c => c.status === 'onHold') && callCount === 1) { holdBtn?.classList.add('active'); if(holdBtn) holdBtn.innerHTML = `<i class="fa-solid fa-play"></i> <span>Resume</span>`; } else if (holdBtn) { holdBtn.classList.remove('active'); holdBtn.innerHTML = `<i class="fa-solid fa-pause"></i> <span>Hold</span>`; } if (appState.isMuted) { muteBtn?.classList.add('active'); doc.getElementById('mute-btn-multi')?.classList.add('active'); doc.getElementById('conf-mute-btn')?.classList.add('active'); } else { muteBtn?.classList.remove('active'); doc.getElementById('mute-btn-multi')?.classList.remove('active'); doc.getElementById('conf-mute-btn')?.classList.remove('active'); } }
}