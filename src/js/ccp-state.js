...
function setIncomingCall(billerId) {
    ...
    if (!biller) {
        ...
        appState.currentCallData = {
            isLive: biller.isLive, billerName: biller.billerName, tla: biller.tla, paymentType: biller.paymentType, adLink: biller.adLink, kbLink: biller.kbLink, ivrNumber: biller.ivrNumber, csrNumber: biller.csrNumber, error: false
        };
...
function acceptCall() {
    ...
    appState.isIncoming = false;
    const newCall = { id: Date.now(), phoneNumber: appState.incomingCallNumber, displayName: appState.currentCallData.billerName, status: 'connected' };
...
function rejectCall() { clearInterval(appState.activeRingtoneInterval); appState.activeRingtoneInterval = null; appState.isIncoming = false; appState.currentCallData = null; logMessage('Call Event', 'Call rejected.'); renderCCP(); }
...
function dialOutbound() { if (!appState.outboundDialNumber) return; if (appState.calls.length === 0) { const newCall = { id: Date.now(), phoneNumber: appState.outboundDialNumber, displayName: appState.outboundDialNumber, status: 'connected' }; appState.calls.push(newCall); startTimer(newCall); logMessage('Call Event', `Outbound call initiated to ${newCall.phoneNumber}`); closeOverlays(); } else { initiateConsult(appState.outboundDialNumber); } appState.outboundDialNumber = ''; }
function initiateConsult(targetNumber) { if (appState.calls.length === 0) return; const activeCall = appState.calls.find(c => c.status === 'connected'); if (activeCall) { activeCall.status = 'onHold'; } const consultCall = { id: Date.now(), phoneNumber: targetNumber, displayName: targetNumber, status: 'connected' }; appState.calls.push(consultCall); startTimer(consultCall); logMessage('Call Event', `Consult call initiated to ${targetNumber}`); closeOverlays(); }
function initiateQuickConnect(qcId) { const qc = appState.quickConnects.find(q => q.id === qcId); if (!qc) { logMessage('Error', `Quick Connect with id ${qcId} not found.`); return; } if (appState.calls.length === 0) { const newCall = { id: Date.now(), phoneNumber: qc.target, displayName: qc.name, status: 'connected' }; appState.calls.push(newCall); startTimer(newCall); logMessage('Call Event', `Outbound call from Quick Connect to ${qc.target}`); closeOverlays(); } else { initiateConsult(qc.target); } }
