...
function setIncomingCall(billerId) {
    ...
    if (!biller) {
        ...
        appState.currentCallData = {
            error: true,
            message: `Biller data for ID ${billerId} could not be found.`
        };
        ...
        appState.currentCallData = {
            ...
            kbLink: biller.kbLink, // ::: UPDATE: kbLink is now passed to the state :::
            ivrNumber: biller.ivrNumber,
            csrNumber: biller.csrNumber,
            notes: biller.notes,
            error: false
        };
        appState.incomingCallNumber = `Call for: ${biller.tla}`;
