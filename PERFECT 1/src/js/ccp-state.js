...
function initializeCCPEvents() {
    ...
    doc.getElementById('multi-call-container')?.addEventListener('click', (e) => {
        // --- FIX START ---
        // The syntax error was here. Changed ".target" to "e.target".
    ...
    doc.getElementById('numpad-overlay')?.addEventListener('click', (e) => {
        if (e.target.matches('.ccp-numpad-grid button')) {
        ...
        } else if (e.target.id === 'numpad-call-btn') {
            dialOutbound();
        } else if (e.target.id === 'numpad-delete-btn') {
