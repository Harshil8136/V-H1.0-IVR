...
function renderCCP() {
    ...
    if (appState.isIncoming) {
        ...
        // ::: UPDATE: Render ACW info from state :::
    ...
    if (appState.activePanel === 'quickConnects') {
        ...
        qcsToShow.forEach(qc => {
            ...
            item.innerHTML = `<i class="fa-solid ${icon}"></i><span>${qc.name}</span>`;
    ...
    if (appState.activePanel === 'settings') {
        ...
        if (ringtoneRadio) ringtoneRadio.checked = true;
        const themeRadio = doc.querySelector(`input[name="theme"][value="${appState.settings.theme}"]`);
        ...
        appState.calls.forEach(call => {
            ...
            let statusText = 'Connected';
             if (call.status === 'onHold') {
                statusText = 'On hold';
            } else if (appState.isConferenced) {
