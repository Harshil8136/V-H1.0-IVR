...
function extractCandidateNumbers(raw) {
    if (!raw) return [];
    // ::: FIX: Corrected the invalid regex to prevent a fatal JavaScript parse error :::
    ...
    // fall back: scan whole string for digit runs if split didn’t find anything
    if (uniques.size === 0) {
...
// ::: UPDATE: This function now initiates a WARM transfer (consult) instead of a blind transfer :::
function transferToNumber(rawTarget, label = 'Destination') {
    ...
    // Only create the consult leg. Do not complete or drop calls.
    // The agent will use the multi-call controls (Swap, Join, Transfer) to proceed.
    initiateConsult(target);
}


// ::: UPDATE: New robust ringtone control functions :::
...
// ::: UPDATE: Timer is now state-driven to fix the "stuck timer" bug :::
function startTimer(call) {
    call.timerInterval = setInterval(() => {
        ...
        if (callInState && (!appState.ccpWindow || !appState.ccpWindow.closed)) {
            ...
            renderCCP(); // Trigger a re-render which will update the timer display
        } else {
            clearInterval(call.timerInterval); // Cleanup if call or window is gone
        }
    }, 1000);
}
...
function setIncomingCall(billerId) {
    ...
    stopRinging(); // Ensure any previous ringtone is stopped
    playRingtone(); // Play the first ring immediately
    appState.activeRingtoneInterval = setInterval(playRingtone, appState.settings.ringtoneInterval * 1000);
    renderCCP();
}

function acceptCall() {
    stopRinging(); // ::: UPDATE: Instantly stops the ringtone :::
    closeOverlays();
    ...
    startTimer(newCall);
    renderCCP(); // Render once to create the timer element
