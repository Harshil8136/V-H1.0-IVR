...
const appState = {
    ...
    lastCallInfo: '',
    incomingCallNumber: '',
    generatedPhoneNumber: '', // Holds the realistic number for the simulation
    currentCallData: null,
    billerData: [],
    areaCodeData: [], // Will hold the new area code database
    quickConnects: [],
    outboundDialNumber: '',
    ...
    settings: {
        ...
        ringtoneInterval: 3,
        // ::: UPDATE: New settings for toggling UI cards :::
        displayBillerInfo: true,
...
// ::: UPDATE: New functions for the Geo-Context Engine :::
function generatePhoneNumberForState(stateAbbr) {
    ...
    let selectedAreaCode = "786"; // A reasonable fallback
...
function lookupAreaCode(phoneNumber) {
    ...
    if (!areaCodeMatch) {
        return appState.areaCodeData[appState.areaCodeData.length - 1]; // Return 'unknown' fallback
    }
    const areaCode = areaCodeMatch[1];
    const foundData = appState.areaCodeData.find(item => item.areaCode === areaCode);
    return foundData || appState.areaCodeData[appState.areaCodeData.length - 1]; // Return found data or fallback
}
...
// ::: UPDATE: setIncomingCall now generates a realistic number and performs a geo-lookup :::
function setIncomingCall(billerId) {
    ...
    if (!biller) {
        ...
        // Generate a realistic phone number and perform the geo-lookup
        ...
        appState.currentCallData = {
            isLive: biller.isLive,
            billerName: biller.billerName,
            tla: biller.tla,
            paymentType: biller.paymentType,
            adLink: biller.adLink,
            kbLink: biller.kbLink,
            ivrNumber: biller.ivrNumber,
            csrNumber: biller.csrNumber,
            // New geo-context data
            callerLocation: locationData,
            callerTime: localTime,
            error: false
        };
...
// ::: UPDATE: The created call object now has a dedicated displayName for the UI :::
function acceptCall() {
    clearInterval(appState.activeRingtoneInterval);
    appState.activeRingtoneInterval = null;
    ...
    const newCall = { 
        id: Date.now(), 
        phoneNumber: appState.generatedPhoneNumber, 
        displayName: appState.currentCallData.billerName,
        status: 'connected' 
    };
...
// --- All other functions (dialOutbound, startACW, etc.) remain unchanged ---
// [Functions from appendToDialNumber to closeOverlays are omitted for brevity but are unchanged]
...
function dialOutbound() { if (!appState.outboundDialNumber) return; if (appState.calls.length === 0) { const newCall = { id: Date.now(), phoneNumber: appState.outboundDialNumber, status: 'connected' }; appState.calls.push(newCall); startTimer(newCall); logMessage('Call Event', `Outbound call initiated to ${newCall.phoneNumber}`); closeOverlays(); } else { initiateConsult(appState.outboundDialNumber); } appState.outboundDialNumber = ''; }
function initiateConsult(targetNumber) { if (appState.calls.length === 0) return; const activeCall = appState.calls.find(c => c.status === 'connected'); if (activeCall) { activeCall.status = 'onHold'; } const consultCall = { id: Date.now(), phoneNumber: targetNumber, status: 'connected' }; appState.calls.push(consultCall); startTimer(consultCall); logMessage('Call Event', `Consult call initiated to ${targetNumber}`); closeOverlays(); }
function initiateQuickConnect(qcId) { const qc = appState.quickConnects.find(q => q.id === qcId); if (!qc) { logMessage('Error', `Quick Connect with id ${qcId} not found.`); return; } if (appState.calls.length === 0) { const newCall = { id: Date.now(), phoneNumber: qc.target, status: 'connected' }; appState.calls.push(newCall); startTimer(newCall); logMessage('Call Event', `Outbound call from Quick Connect to ${qc.target}`); closeOverlays(); } else { initiateConsult(qc.target); } }
