...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadData = () => {
        // ::: UPDATE: Now loads area code data as well :::
        if (typeof billerData !== 'undefined' && typeof quickConnectsData !== 'undefined' && typeof areaCodeData !== 'undefined') {
            appState.billerData = billerData;
            appState.quickConnects = quickConnectsData;
            appState.areaCodeData = areaCodeData; // Load area code data into state
            ...
            logMessage('System', `${appState.billerData.length} billers & ${appState.areaCodeData.length} area codes loaded.`);
        } else {
            logMessage('Error', 'Data files not found. Ensure biller-data.js and area-code-data.js are loaded.');
        }
    };
    ...
    const initializeCCPEvents = () => {
        ...
        // General listeners (status, header, overlays)
        ...
        // ::: UPDATE: Add event listener for new display toggles :::
        ...
        // Event delegation and other listeners are omitted for brevity but are unchanged
